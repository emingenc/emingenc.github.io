  ...
  <div v-else class="grid gap-4 grid-cols-1 sm:grid-cols-3">
    ...
    >
      <input
        ref="share-url"
        readonly
        :value="`https://emingenc.gitthub.io${path}`"
    ...
    share(option: "url" | "twitter" | "telegram" | "whatsapp") {
      if (option === "url") {
        ...
        if (!el) {
          ...
          el.value = this.path ? `https://emingenc.gitthub.io${this.path}` : location.href
        ...
        switch (option) {
          case "twitter":
            url = `https://twitter.com/intent/tweet?via=emingench&text=${encodeURIComponent(
              this.title + "\n" + location.href
            )}`
            break
          case "telegram":
